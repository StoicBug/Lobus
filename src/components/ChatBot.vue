...
let toogleChatValue = ref(false)
let messages = ref([])

let toogleChat = () => {
  toogleChatValue.value = !toogleChatValue.value
}
...
const sendMessage = (event) => {
  ...
  if (input) {
    const userMessage = { isbot: false, body: input }
    ...
    getChatbotResponse(input)
      .then(response => {
        const botMessage = { isbot: true, body: response }
...
const getChatbotResponse = async (input) => {
  const apiEndpoint = 'http://localhost:8080/chat';
  const queryParam = `?query=${encodeURIComponent(input)}`;
  const url = `${apiEndpoint}${queryParam}`;
...
<template>
  <div class="z-50">
    <button
      ...
      type="button" @click="toogleChat" aria-haspopup="dialog" aria-expanded="false" data-state="closed">
    ...
    <div style="box-shadow: 0 0 #0000, 0 0 #0000, 0 1px 2px 0 rgb(0 0 0 / 0.05);"
      :class="toogleChatValue ? 'block' : 'hidden'"
      class="z-50 fixed bottom-[calc(4rem+1.5rem)] right-0 mr-4 bg-white p-6 rounded-lg border border-[#e5e7eb] w-[440px] h-[634px]">

      <!-- Heading -->
      <div class="flex flex-col space-y-1.5 pb-6">
        <h2 class="font-semibold text-lg tracking-tight">Chatbot</h2>
        <p class="text-sm text-[#6b7280] leading-3">Powered by Lobus</p>
      </div>
      ...
      <!-- Chat Container -->
      <div class="pr-4 flex flex-col h-[474px] overflow-y-auto" style="min-width: 100%; ">
        <div v-for="message in messages" :key="message.body">
          <div v-if="message.isbot" class="flex gap-3 my-4 text-gray-600 text-sm flex-1">
            ...
            <p class="leading-relaxed"><span class="block font-bold text-gray-700">AI </span>{{ message.body }}</p>
          </div>
          <div v-else class="flex gap-3 my-4 text-gray-600 text-sm flex-1">
            ...
            <p class="leading-relaxed"><span class="block font-bold text-gray-700">You </span>{{ message.body }}</p>
          </div>
        </div>
      </div>
