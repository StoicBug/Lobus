<template>
    <div class="relative flex flex-col items-center max-w-lg gap-4 p-6 rounded-md shadow-md sm:py-8 sm:px-12 dark:bg-gray-900 dark:text-gray-100"
        :class="{ hidden: showParticulierModal }">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"
            class="w-40 h-40 fill-current shrink-0 dark:text-violet-400">
            <path
        ...
        <h2 class="text-2xl font-semibold leading-tight tracking-wide">Quis vel eros donec ac odio tempor.</h2>
        <p class="flex-1 text-center dark:text-gray-400">Tempora ipsa quod magnam non, dolores nemo optio. Praesentium
            soluta maiores non itaque aliquam sint.</p>
        <button type="button" class="px-8 py-3 font-semibold rounded-full dark:bg-violet-400 dark:text-gray-900">Start
            recycling</button>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            showParticulierModal: this.show
        }
    },
    methods: {
        closeParticulierModal() {
            this.showParticulierModal = false
        }
    }
}
</script>